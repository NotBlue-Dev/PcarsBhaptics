...
class BHapticsTactJsAdapter {
    ...
    playEffect(name, options) {
        return tactJs.default.submitRegisteredWithScaleOption(name, options)
    }

    playPath(position,points,dur) {
        return tactJs.default.submitPath('dot', position, points, dur);
    }
